...
# 使用一元线性回归模型对droupout进行测试
# 设置了两个相同的模型，一个使用droupout，一个不使用droupout进行对比分析
# ============================ step 1/5 数据 ============================
...
class MLP(nn.Module):
    def __init__(self, neural_num, d_prob=0.5):
        ...
        self.linears = nn.Sequential(
            ...
            nn.Linear(1, neural_num),           # 输入层
            nn.ReLU(inplace=True),

            nn.Dropout(d_prob),                 # droupout放在需要进行随机舍弃的层的前一层
...
# 构造两个模型，第一个不进行droupout，第二个进行droupout
...
for epoch in range(max_iter):
    ...
    if (epoch+1) % disp_interval == 0:
        # =========================== droupout使用的说明 ===========================
        # 由于droupout在训练阶段每个神经元都会随机的失活，而测试阶段是所有的神经元都参加
        # 因此在进行测试之前需要对网络的状态进行设置
        # net.eval()表示网络进入测试状态，当测试完成后使用net.train()将网络切换回训练状态
        # ==========================================================================
